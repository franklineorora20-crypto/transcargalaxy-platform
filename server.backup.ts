...
function requireRole(
  ...roles: Array<'admin' | 'driver' | 'customer'>
) {
  return (
    req: Request,
    res: Response,
...
const requireManager = requireRole('admin');
...
async function createDriverAccount(
  ...
  if (
    ...
    throw new Error(
      'Supabase admin provisioning is not configured.',
    );
  }
...
app.post(
  ...
  async (req, res) => {
    ...
    if (
      ...
      return res.status(400).json({
        error:
          'Name, a username (3–32 letters, numbers, dots, underscores, or hyphens), phone, licence details, and a password of at least 12 characters are required.',
      });
    }

    if (
      Number.isNaN(
        Date.parse(licenseExpiry),
      )
    ) {
      return res.status(400).json({
        error:
